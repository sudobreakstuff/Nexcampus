...
TOOL_INIT.grapher = function() { initGraph(); };
...
function initGraph() {
  ...
  c.addEventListener('wheel', function(e) {
    ...
    GRAPH.zoom = Math.max(0.1, Math.min(20, GRAPH.zoom * d));
    graphDraw();
  }, {passive:false});
  c.addEventListener('mousedown', function(e) {
    GRAPH.dragging = true; GRAPH.dragX = e.clientX; GRAPH.dragY = e.clientY;
    GRAPH.startPanX = GRAPH.panX; GRAPH.startPanY = GRAPH.panY;
  });
  c.addEventListener('mousemove', function(e) {
    if (!GRAPH.dragging) return;
    GRAPH.panX = GRAPH.startPanX + (e.clientX - GRAPH.dragX) * 700 / c.getBoundingClientRect().width;
    GRAPH.panY = GRAPH.startPanY + (e.clientY - GRAPH.dragY) * 400 / c.getBoundingClientRect().height;
    graphDraw();
  });
  c.addEventListener('mouseup', function() { GRAPH.dragging = false; });
  c.addEventListener('mouseleave', function() { GRAPH.dragging = false; });
  if (GRAPH.funcs.length === 0) {
    GRAPH.funcs.push({expr:'sin(x)', color:'#4fc3f7'});
...
function graphPlot() {
  var input = document.getElementById('graph-func');
  if (!input || !input.value.trim()) return;
  var colors = ['#4fc3f7','#ff8a65','#81c784','#f06292','#ba68c8','#ffd54f','#4dd0e1','#ffab40'];
  GRAPH.funcs.push({expr:input.value.trim(), color:colors[GRAPH.funcs.length % colors.length]});
  input.value = '';
  graphDraw();
}

function graphClear() {
  GRAPH.funcs = []; GRAPH.zoom = 1; GRAPH.panX = 0; GRAPH.panY = 0;
  graphDraw();
}
...
function graphExport() {
  ...
  link.href = GRAPH.canvas.toDataURL();
  link.click();
}

function graphDraw() {
  var ctx = GRAPH.ctx, c = GRAPH.canvas, w = 700, h = 400;
  if (!ctx) return;
  ctx.fillStyle = '#0a0e14';
  ctx.fillRect(0, 0, c.width, c.height);

  var zoom = GRAPH.zoom, px = GRAPH.panX, py = GRAPH.panY;
  var cx = w/2 + px, cy = h/2 + py;
  var scale = 30 * zoom;
  ...
  ctx.lineWidth = 0.5;
  var gs = scale;
  var startX = Math.floor((-cx) / gs) * gs + cx;
  for (var x = startX; x < w; x += gs) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, h); ctx.stroke(); }
  var startY = Math.floor((-cy) / gs) * gs + cy;
  for (var y = startY; y < h; y += gs) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(w, y); ctx.stroke(); }
  ...
  // Plot functions
  GRAPH.funcs.forEach(function(f, idx) {
    ...
    for (var px = 0; px < w; px += 1) {
      ...
      var yv;
      try {
        yv = Function('x', 'return ' + f.expr.replace(/\^/g, '**'))(xv);
      } catch(e) { continue; }
      if (isNaN(yv) || !isFinite(yv)) { first = true; continue; }
      var pyVal = cy - yv * scale;
      if (pyVal < -1000 || pyVal > h + 1000) { first = true; continue; }
      if (first) { ctx.moveTo(px, pyVal); first = false; }
      else ctx.lineTo(px, pyVal);
    }
    ctx.stroke();
  });
}
...
// ==== Formula Reference ====
var FORMULAS = [
  {cat:'Math', name:'Quadratic Formula', expr:'x = (-b ± √(b² - 4ac)) / 2a', desc:'Solves ax² + bx + c = 0'},
  {cat:'Math', name:'Pythagorean Theorem', expr:'a² + b² = c²', desc:'Right triangle side relation'},
  {cat:'Math', name:'Area of Circle', expr:'A = πr²', desc:'Area from radius r'},
  {cat:'Math', name:'Slope', expr:'m = (y₂ - y₁) / (x₂ - x₁)', desc:'Rate of change between two points'},
  {cat:'Math', name:'Distance Formula', expr:'d = √((x₂-x₁)² + (y₂-y₁)²)', desc:'Distance between two points'},
  {cat:'Math', name:'Binomial Theorem', expr:'(a + b)ⁿ = Σ C(n,k)·aⁿ⁻ᵏ·bᵏ', desc:'Expansion of binomials'},
  {cat:'Math', name:'Derivative Power Rule', expr:'d/dx [xⁿ] = n·xⁿ⁻¹', desc:'Basic derivative of powers'},
  {cat:'Math', name:'Natural Log Derivative', expr:'d/dx [ln(x)] = 1/x', desc:'Derivative of ln'},
  {cat:'Math', name:'Sum of Series', expr:'Σᵢ₌₁ⁿ i = n(n+1)/2', desc:'Sum of 1 to n'},
  {cat:'Physics', name:"Newton's 2nd Law", expr:'F = m·a', desc:'Force equals mass times acceleration'},
  {cat:'Physics', name:'Kinetic Energy', expr:'KE = ½mv²', desc:'Energy of motion'},
  {cat:'Physics', name:'Gravity Force', expr:'F = G·m₁·m₂ / r²', desc:'Gravitational attraction (G = 6.67×10⁻¹¹)'},
  {cat:'Physics', name:'Ohm\'s Law', expr:'V = I·R', desc:'Voltage = current × resistance'},
  {cat:'Physics', name:'Wave Speed', expr:'v = f·λ', desc:'Speed = frequency × wavelength'},
  {cat:'Physics', name:'Work', expr:'W = F·d·cos(θ)', desc:'Work = force × distance × cos(angle)'},
  {cat:'Physics', name:'Impulse', expr:'J = F·Δt = Δp', desc:'Impulse = change in momentum'},
  {cat:'Physics', name:'Doppler Effect', expr:'f\' = f·(v±vₒ)/(v∓vₛ)', desc:'Observed frequency shift'},
  ...
  {cat:'Chemistry', name:'pH Formula', expr:'pH = -log₁₀[H⁺]', desc:'Acidity from hydrogen ion concentration'},
  {cat:'Chemistry', name:'Henderson-Hasselbalch', expr:'pH = pKa + log([A⁻]/[HA])', desc:'Buffer pH calculation'},
  {cat:'Chemistry', name:'Half-Life', expr:'t₁/₂ = ln(2) / k', desc:'Time for half the substance to decay'},
  {cat:'Chemistry', name:'Percent Yield', expr:'% Yield = (actual / theoretical)×100', desc:'Reaction efficiency'},
  {cat:'Stats', name:'Mean', expr:'μ = Σxᵢ / n', desc:'Average of values'},
  {cat:'Stats', name:'Standard Deviation', expr:'σ = √(Σ(xᵢ-μ)²/n)', desc:'Spread of data around mean'},
  {cat:'Stats', name:'Probability (Independent)', expr:'P(A∩B) = P(A)·P(B)', desc:'Both events occurring'},
  {cat:'Stats', name:'Binomial Probability', expr:'P(k) = C(n,k)·pᵏ·(1-p)ⁿ⁻ᵏ', desc:'k successes in n trials'},
  {cat:'CS', name:'Time Complexity (Nested)', expr:'O(n²)', desc:'Nested loops over n items'},
  {cat:'CS', name:'Binary Search', expr:'O(log n)', desc:'Halving search space each step'}
];
...
function formulaSearch() {
  ...
  var cats = {};
  results.forEach(function(f) { if (!cats[f.cat]) cats[f.cat] = []; cats[f.cat].push(f); });
  var html = '';
