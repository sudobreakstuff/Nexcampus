...
function formulaSearch() {
  ...
  out.innerHTML = html || '<div style="color:var(--fg-dim);padding:12px">No formulas found. Try quadratic or newton.</div>';
}
var FORMULAS = [
  {cat:"Math", name:"Quadratic Formula", expr:"x = (-b +/- sqrt(b2 - 4ac)) / 2a", desc:"Solves ax2 + bx + c = 0"},
  {cat:"Math", name:"Pythagorean Theorem", expr:"a2 + b2 = c2", desc:"Right triangle: hypotenuse c, legs a and b"},
  {cat:"Math", name:"Area of a Circle", expr:"A = pi * r2", desc:"Area from radius. pi ~ 3.14159"},
  {cat:"Math", name:"Slope Formula", expr:"m = (y2 - y1) / (x2 - x1)", desc:"Rate of change between two points"},
  {cat:"Math", name:"Distance Formula", expr:"d = sqrt((x2-x1)2 + (y2-y1)2)", desc:"Distance between two points"},
  {cat:"Math", name:"Derivative of xn", expr:"d/dx[xn] = n*x(n-1)", desc:"Power rule for differentiation"},
  {cat:"Math", name:"Sum of 1 to n", expr:"sum = n(n+1)/2", desc:"Arithmetic series: 1+2+...+n"},
  {cat:"Math", name:"Logarithm Product Rule", expr:"log(ab) = log(a) + log(b)", desc:"Log of product equals sum of logs"},
  {cat:"Physics", name:"Force F = ma", expr:"F = m * a", desc:"Newtons 2nd Law: force = mass * acceleration"},
  {cat:"Physics", name:"Kinetic Energy", expr:"KE = mv2 / 2", desc:"Energy of a moving object = half m v squared"},
  {cat:"Physics", name:"Gravitational Force", expr:"F = G*m1*m2 / r2", desc:"Gravity between two masses, G = 6.67e-11"},
  {cat:"Physics", name:"Ohms Law", expr:"V = I * R", desc:"Voltage equals current times resistance"},
  {cat:"Physics", name:"Wave Speed", expr:"v = f * lambda", desc:"Speed equals frequency times wavelength"},
  {cat:"Physics", name:"Work Formula", expr:"W = F * d * cos(theta)", desc:"Work = force times distance times cos(angle)"},
  {cat:"Physics", name:"Density", expr:"density = mass / volume", desc:"How much mass fits in a given volume"},
  {cat:"Chemistry", name:"Ideal Gas Law", expr:"PV = nRT", desc:"Pressure*volume = moles * R * temperature. R=8.314"},
  {cat:"Chemistry", name:"Molarity", expr:"M = n / V", desc:"Moles of solute per liter of solution"},
  {cat:"Chemistry", name:"pH Formula", expr:"pH = -log[H+]", desc:"Acidity measure from hydrogen ion concentration"},
  {cat:"Chemistry", name:"Percent Yield", expr:"yield% = actual/theoretical * 100", desc:"Reaction efficiency"},
  {cat:"Chemistry", name:"Half-Life", expr:"t-half = ln(2) / k", desc:"Time for half to decay, k = decay constant"},
  {cat:"Stats", name:"Mean Average", expr:"mean = sum(xi) / n", desc:"Arithmetic average of n data points"},
  {cat:"Stats", name:"Standard Deviation", expr:"sigma = sqrt(sum(xi-mean)2/n)", desc:"How spread out the data is"},
  {cat:"Stats", name:"Independent Events", expr:"P(A and B) = P(A) * P(B)", desc:"Probability both events happen"},
  {cat:"CS", name:"Binary Search", expr:"O(log n)", desc:"Split search space in half each step"},
  {cat:"CS", name:"Nested Loop", expr:"O(n2)", desc:"Two nested loops = quadratic time"}
];
...
function formulaSearch() {
  ...
  // Build with DOM (no quote issues)
  var cats = {};
  results.forEach(function(f) {
    if (!cats[f.cat]) cats[f.cat] = [];
    cats[f.cat].push(f);
  ...
  Object.keys(cats).forEach(function(cat) {
    ...
    catDiv.style.cssText = 'font-weight:bold;color:var(--cyan);font-size:11px;margin:10px 0 4px;border-bottom:1px solid var(--border);padding-bottom:2px';
    catDiv.textContent = cat;
    out.appendChild(catDiv);
    cats[cat].forEach(function(f) {
      var itemDiv = document.createElement('div');
      itemDiv.style.cssText = 'padding:4px 0;border-bottom:1px solid rgba(255,255,255,0.03)';
      var nameSpan = document.createElement('span');
      nameSpan.style.cssText = 'color:var(--fg);font-weight:bold;font-size:10px';
      nameSpan.textContent = f.name;
      itemDiv.appendChild(nameSpan);
      var exprDiv = document.createElement('div');
      exprDiv.style.cssText = 'font-family:monospace;font-size:11px;color:var(--teal);margin:2px 0';
      exprDiv.textContent = f.expr;
      itemDiv.appendChild(exprDiv);
      var descDiv = document.createElement('div');
      descDiv.style.cssText = 'font-size:10px;color:var(--fg-dim)';
  ...
  if (!out.firstChild) {
    ...
    empty.style.cssText = 'color:var(--fg-dim);padding:12px';
    empty.textContent = 'No formulas found. Try "quadratic" or "newton" or "gas".';
